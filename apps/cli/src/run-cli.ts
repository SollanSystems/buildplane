...
function formatTopLevelHelp(): string[] {
	return [
		BUILDPLANE_BANNER,
		"Subcommand CLI — this does not open an interactive shell.",
		"Usage:",
		"  buildplane init",
		"  buildplane run --packet <path> [--tui]",
		"  buildplane status [--json]",
		"  buildplane inspect <run-id> [--json]",
		"  buildplane history [--json]",
		"  buildplane replay <run-id> [--json]",
		"  buildplane run-graph --graph <path>",
		"  buildplane run-strategy --strategy <path> [--json]",
		"  buildplane memory <action> ...",
