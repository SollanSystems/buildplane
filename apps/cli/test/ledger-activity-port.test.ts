...
} from "../src/ledger-activity-port.js";
...
describe("createDeferredLedgerActivityPort", () => {
	it("no-ops when the emitter getter returns null (non-ledger run)", async () => {
		const emitter = createFakeEmitter();
		const bound: TapeEmitter | null = null;
		const port = createDeferredLedgerActivityPort(() => bound);
		...
		expect(emitter.emits).toHaveLength(0);
		expect(emitter.flushCount).toBe(0);
	});

	it("emits + write-ahead flushes once the signed emitter is bound", async () => {
		const emitter = createFakeEmitter();
		let bound: TapeEmitter | null = null;
		const port = createDeferredLedgerActivityPort(() => bound);

		// run block binds the signed emitter before any activity fires
		bound = emitter.emitter;
		...
		expect(emitter.emits).toHaveLength(1);
		expect(emitter.emits[0]?.kind).toBe("activity_started");
		expect(emitter.flushCount).toBe(1);
		expect(emitter.flushAtSeq[0]).toBe(1);
	});
});
