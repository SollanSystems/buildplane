...
| `bp web` Mission Control approval inbox — `POST /api/runs/:runId/decision` | **Broken.** Every approve/reject call returns HTTP 500 and applies no side effect. | No | No | **Source/dev-only; fails closed.** The default-wired `OperatorDecisionPort` (`run-cli.ts:5289`) spawns `ledger serve --sign` and emits `operator_decision_recorded`, which the signed-append denylist in `reject_caller_supplied_authority_event` (`native/crates/bp-ledger/src/serve.rs:324`) rejects. The identically-wired `RunCompletionPort` (`run-cli.ts:5296`, `run_completed` denylisted at `serve.rs:309`) fails the same way but is masked behind the decision failure. The break is fail-closed: `orchestrator.ts:4952`'s `await operatorDecisionPort.recordDecision(input)` throws before the Tier-1 shadow row (`:4957`) or the side effect (`:4968`) ever runs, so no false evidence is produced and no partial merge occurs. A published install never reaches this code path — `bp web` is source/dev-only and exits 1 with source-checkout guidance before serving any route. Coverage: until this entry landed, no test anywhere referenced `createOperatorDecisionPort` or `createRunCompletionPort`; `test/ledger-integration/operator-decision-writers.test.ts` now pins the rejection and the empty-tape fail-closed property, and must be updated rather than deleted when this surface is resolved. The surviving router test (`packages/mission-control-server/test/router.test.ts:56`) mocks `recordOperatorDecision` rather than exercising the real port, so it cannot detect this regardless of pass state. Regressed by #281 (`a53519b`), which added this denylist and, in the same commit, deleted the three `ledger-integration` tests that had covered these writers (`operator-decision-merge`, `operator-decision-resume`, `run-completed-emit`) — neither the break nor the coverage loss is disclosed in the PR description or its changeset. **Open, unresolved:** whether this surface is intended to be retired (in which case it wants an explicit guard in the style of `blockPlanForgeLegacyExecution`) or is an unintended regression (in which case it wants a dedicated native control, per the denylist's own doc comment at `serve.rs:246-250`: "Governed code must use a dedicated control that derives those records from trusted state instead"). Removing `operator_decision_recorded`/`run_completed` from the denylist is not a fix — it reopens the hole the denylist exists to close. |
