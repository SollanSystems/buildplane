...
| `buildplane run --resume <promotion-request-event> --approve --decision promote\|reject` | Protected decision and separate promotion-execution clients integrated; deployment gated | Reject records no Git effect; promote may enter one native replay-derived CAS | Signed decision and protected promotion status; target mutation only for the exact sealed promote decision | Deploy both role-specific sockets and hosts from the protected bundle. Reconcile any non-recorded execution status; never resubmit blindly. |
