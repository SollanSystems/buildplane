...
| `buildplane run --packet <file> --approve` | Fixed protected-host client integrated; deployment gated | Only when the enrolled host proves authority and returns a verified candidate receipt | Candidate evidence only; never target mutation | Deploy/enroll the protected authority host and OCI action plane; see the governed-run runbook. |
| `buildplane run --resume <opaque-ref> --approve` | Fixed protected-host recovery integrated; deployment gated | Only on a deployed protected host; never a local retry | Only the exact verified host result; never target mutation | Preserve the opaque reference and use the enrolled protected host to reconcile the existing workflow; never submit a replacement packet. |
