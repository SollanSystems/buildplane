...
export function addNativeRfc3339UtcMilliseconds(
	...
	return addNativeRfc3339UtcNanoseconds(
		value,
		BigInt(milliseconds) * NANOS_PER_MILLISECOND,
	);
}

/**
 * Advance a native RFC3339 UTC timestamp using Chrono-compatible ordering.
 * This is deliberately exposed for authority-clock uncertainty bounds: a
 * millisecond JavaScript clock denotes an interval, while a native dispatch
 * may contain up to nanosecond precision.
 */
export function addNativeRfc3339UtcNanoseconds(
	value: unknown,
	nanoseconds: bigint,
): bigint | undefined {
	const parsed = parseNativeRfc3339UtcDetails(value);
	if (parsed === undefined || nanoseconds < 0n) return undefined;

	const secondsToAdd = nanoseconds / NANOS_PER_SECOND;
	const fractionalNanosToAdd = nanoseconds % NANOS_PER_SECOND;
	if (secondsToAdd > BigInt(Number.MAX_SAFE_INTEGER)) return undefined;
