...
export function createStorageStore(projectRoot: string): Omit<
	BuildplaneStoragePort,
	"initializeProject"
> & {
	...
	return {
		...
		inspectTarget(id: string): InspectSnapshot {
			...
			if (runRow) {
				const snapshot = runRow.unit_snapshot
					? JSON.parse(runRow.unit_snapshot)
					: null;
				// Handle both old format (Unit only) and new format (full UnitPacket)
				const unit: Unit =
					snapshot && "unit" in snapshot
						? (snapshot.unit as Unit)
						: snapshot
							? (snapshot as Unit)
		...
		getPacketSnapshot(runId: string): UnitPacket | null {
			...
			// New format: full UnitPacket with { unit, execution/model, verification }
			if (parsed && "unit" in parsed && "verification" in parsed) {
				return parsed as UnitPacket;
			}
			// Old format: just the Unit — can't reconstruct full packet
			return null;
		},
	};
}
