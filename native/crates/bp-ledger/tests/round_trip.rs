...
#[test]
fn unsigned_event_remains_readable_after_signature_table_is_added() {
    let store = SqliteStore::open_in_memory().unwrap();
    let run_id = RunId::new();
    let event = build(
        run_id,
    ...
    assert_eq!(rows[0].id, event.id.to_string());
    assert_eq!(
        signature_count, 0,
...
fn payload_round_trips_through_canonicalize() {
    ...
    let canonical =
        canonicalize_payload(&rows[0].kind, rows[0].schema_version, payload_json).unwrap();
